...
- A protected browser stress sample with 8,000 organisms, Forked Mind, Helix Guard, Sister Broods and a boss reported approximately 60 fps, 6.8 ms of simulation work per frame, frame P99 18.3 ms and zero WebGL errors. This is a local rolling sample, not a controlled cross-hardware benchmark.
- CI repeats the native tests, full Forked Mind survival route, production build and browser regression before deploying `main` to GitHub Pages. The browser regression uses a 960 × 640 viewport at 0.5 pixel density because the hosted runner renders WebGL on its CPU. It retains the actual renderer, simulation and audio lifecycle assertions; visual/performance playtests use the normal app-browser resolution.
